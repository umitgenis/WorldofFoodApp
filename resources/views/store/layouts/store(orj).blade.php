...
{{--                        <div class="col-auto">--}}
                            <div
                                style="/*width: 30vi; white-space: nowrap;overflow: hidden; text-overflow: ellipsis;*/ border-radius: 5px; opacity: 0.7"
                                class="mx-auto pt-lg-0   bg-soft-danger me-2 mt-md-1">
                                <p class="mb-0 fw-bold text-lg-center p-2 text-responsive">Deliver to: <i
                                        class="fas fa-map-marker-alt text-warning mx-2"></i><span
                                        class="fw-normal">Current Location </span><span><i>Semih Ev</i>, Laledere Çiftlik Köy No:12-A, Yalova </span>
                                </p>
                            </div>
{{--                        </div>--}}
{{--                        <div class="col-auto">--}}
                            <form action="{{route('store.search.restaurant')}}" class="d-inline-flex mt-2 mt-lg-0 ms-lg-auto ms-xl-0">
                                @csrf
                                <div class="input-group-icon pe-sm-2 flex-1">
                                    <i class="fas fa-search input-box-icon text-primary"></i>
                                    <input style="" class="form-control border-0 input-box bg-150 border-100" type="search" name="search"
                                           placeholder="Search Restaurant" aria-label="Search"
                                        {{--                               value="Search Restaurant"--}}
                                        {{--                               onfocus="this.value = '';"--}}
                                        {{--                               onblur="if (this.value === '') {this.value = 'Search Restaurant';}"--}}
                                    />
                                </div>
                            </form>
                            @guest()
                                <a href="{{route('login')}}" class="d-inline-flex mt-2 mt-lg-0 ms-lg-auto ms-xl-0 mx-sm-2 d-sm-inline-flex">
                                    <button class="btn btn-white shadow-warning text-warning border-100 px-3 px-sm-1" type="submit">
                            ...
                            @auth()
                                <form action="{{route('logout')}}" method="POST" class="d-sm-inline-flex mt-2 mt-lg-0 ms-lg-auto ms-xl-0 mx-sm-2">
                                    @csrf
                                    <button class="btn btn-white shadow-warning text-warning border-100  px-sm-1" type="submit">
                                        <i class="fas fa-user me-2 me-sm-1"></i>Logout
                                    </button>
                                </form>
                            @endauth
                            <a href="#" class="d-sm-inline-flex">
                                <button class="btn btn-white shadow-warning text-warning border-100  px-sm-1 " type="submit">
                                    <i class="fa fa-shopping-basket me-2 me-sm-1" aria-hidden="true" ></i>Basket
                                </button>
                            </a>
{{--                        </div>--}}
{{--                    </div>--}}
